...
* Optionally: install and configure PHP7-FPM (FastCGI)
* Optionally: install PHP-MySQL connector and/or others PHP7 modules
* Basic vhosts
* Proxy vhosts
...
Nginxpack creates this default vhost for you, with a default certificate. To replace the default certificate, you can use `ssl_default_cert_source` (or `ssl_default_cert_key`) and `ssl_default_key_source` (or `ssl_default_key_source`) options. This certificate should be valid for all domains used, so it will probably be at least a wildcard certificate. In a same manner, you can also define a `ssl_default_dhparam_source` or `ssl_default_dhparam_content` parameter.
